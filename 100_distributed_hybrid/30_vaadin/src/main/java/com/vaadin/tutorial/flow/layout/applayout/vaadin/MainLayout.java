...
import com.vaadin.tutorial.flow.layout.applayout.vaadin.views.usermanagement.UserManagementView;
...
public class MainLayout
  ...
  public static final String ITM_USER_MANAGEMENT = "mainview.menue.item.usermanagement";
  ...
  public static final String ITM_PROJECTS        = "mainview.menue.item.projects";
  public static final String ITM_LOGOUT          = "mainview.menue.item.logout";

  private Map<Tab, Component> tab2Workspace = new HashMap<>();

  public MainLayout() {
    StreamResource res = new StreamResource(LOGO_PNG, () -> MainView.class.getResourceAsStream("/" + LOGO_PNG));
    Image          img = new Image(res, "Vaadin Logo");
    img.setHeight("44px");
    addToNavbar(new DrawerToggle(), img);

    final Tabs tabs = new Tabs(dashBoard(), userProfile(), userManagement(), projects(), logout());
  ...
  private Tab logout() {
    ...
    btn.addClickListener(e -> {
      ...
      session.setAttribute(SessionUser.class, null);
      session.close();
...
//    tab2Workspace.put(tab, new ProjectView());
//    return tab;
//  }

  private Tab projects() {
    final Span label = new Span(getTranslation(ITM_PROJECTS));
    final Icon icon  = TASKS.create();
    final Tab  tab   = new Tab(new HorizontalLayout(icon, label));
    tab2Workspace.put(tab, new ProjectView());
    return tab;
  }
  ...
  private Tab userManagement() {
    final Span label = new Span(getTranslation(ITM_USER_MANAGEMENT));
    final Icon icon  = USERS.create();
    final Tab  tab   = new Tab(new HorizontalLayout(icon, label));
    tab2Workspace.put(tab, new UserManagementView());
    return tab;
  }

  private Tab userProfile() {
